...
export function useAI() {
  ...
  // Use a ref to always have the latest state in async callbacks
  ...
  const streamMessage = useCallback(
    async (
      ...
      const language =
        currentState.settings.defaultLanguage || 'Python';

      const hasAnthropicKey = !!currentState.apiKey;

      const hasGeminiKey = !!(
        currentState.settings as any
      )?.geminiApiKey;
...
This takes 2 minutes and gives you unlimited usage.`;
      ...
      try {
        ...
        const academicContext =
          profile?.college && profile?.course
            ? `\n\n[STUDENT CONTEXT]: The student is studying ${profile.course} at ${profile.college}${
                profile.year
                  ? `, Year ${profile.year}`
                  : ''
              }. Please tailor your explanations and examples to be relevant to this curriculum and level.`
            : '';

        const baseSystemPrompt = session.attachedFile
          ? buildSystemPromptWithFile(
              truncateForContext(
                session.attachedFile.text
              ),
        ...
        const finalSystemPrompt =
          baseSystemPrompt + academicContext;

        // Priority:
        // Anthropic → Gemini → Built-in Groq
        if (
          currentState.settings?.provider ===
            'anthropic' &&
          hasAnthropicKey
        ) {
  ...
  const generateQuiz = useCallback(
    async (
      ...
      const language =
        stateRef.current.settings.defaultLanguage ||
        'Python';

      const hasAnthropicKey =
        !!stateRef.current.apiKey;

      const hasGeminiKey = !!(
        stateRef.current.settings as any
      )?.geminiApiKey;
      ...
      try {
        ...
        const academicContext =
          profile?.college && profile?.course
            ? ` for a student studying ${profile.course} at ${profile.college}${
                profile.year
                  ? `, Year ${profile.year}`
                  : ''
              }`
        ...
        if (
          stateRef.current.settings?.provider ===
            'anthropic' &&
          hasAnthropicKey
        ) {
          const client = getAnthropicClient(
        ...
        if (!match) {
          throw new Error(
            'Invalid quiz response. Please try again.'
          );
        }
